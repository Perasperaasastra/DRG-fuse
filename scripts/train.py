...
def main() -> None:
    ...
    # Optional early stopping (manuscript: "val is used for early stopping / model selection").
    patience = int(cfg["train"].get("patience", 0))  # 0 disables early stopping
    ...
    for epoch in range(1, epochs + 1):
        ...
        improved = (np.isfinite(paucn) and paucn > best_key + min_delta)
        if improved:
        ...
        if patience > 0 and no_improve >= patience:
            print(f"Early stopping at epoch={epoch} (best_epoch={best_epoch}, best_pAUCn={best_key:.4f}).")
            break
