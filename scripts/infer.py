...
def main() -> None:
    ...
    ap.add_argument("--data_root", type=str, default="data/example",
                    help="Root folder containing metadata.csv and feature files. Default: data/example")
    ap.add_argument("--sample_id", type=str, default=None,
                    help="Sample id in metadata.csv. If omitted, use the first row in metadata.csv.")
    args = ap.parse_args()

    data_root = Path(args.data_root)
    if args.sample_id is None:
        meta = pd.read_csv(data_root / "metadata.csv")
        if "sample_id" not in meta.columns or len(meta) == 0:
            raise RuntimeError("metadata.csv must contain a non-empty 'sample_id' column.")
        args.sample_id = str(meta.iloc[0]["sample_id"])
        print(f"[infer] --sample_id not provided; defaulting to first row: {args.sample_id}")
