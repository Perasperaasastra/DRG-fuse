...
5. **Inference code + inference example**  
   ...
   - a locally generated toy checkpoint under `outputs/`  
...
9. **A model checkpoint that inference can load**  
   - a locally generated toy checkpoint (trained only on synthetic toy data)
...
```bash
python scripts/infer.py --checkpoint <PATH_TO_BEST_PT> --data_root data/example --sample_id S0001
```
